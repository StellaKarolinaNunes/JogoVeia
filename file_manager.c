...
static void applyCrypto(char *data) {
  ...
  for (int i = 0; data[i] != '\0'; i++) {
    if (data[i] == '\n' || data[i] == '\r')
      continue; // Manter quebras de linha legíveis para fgets mas conteúdo
                // cifrado
    data[i] ^= CRYPTO_KEY;
  }
}
...
void carregarRanking(void) {
  ...
  while (num_ranking_entries < MAX_RANKING_ENTRIES &&
         fread(&ranking[num_ranking_entries], sizeof(RankingEntry), 1, file) ==
             1) {
    num_ranking_entries++;
  }
...
void atualizarRanking(const char *player_name, int result_type) {
  ...
  switch (result_type) {
  case 1:
    ranking[found_index].wins++;
    break;
  case 0:
    ranking[found_index].ties++;
    break;
  case -1:
    ranking[found_index].losses++;
    break;
  }
}
...
void salvarJogo(GameState *game) {
  ...
  if (selected == 1) { // Sobrescrever
    ...
    while (1) {
      exibirTelaSavesList(available_saves, num_available_saves, sel_idx,
                          0); // mode 0 = sobrescrever
      KeyCode tecla = lerTeclaMenu();
      if (num_available_saves == 0) {
        if (tecla == KEY_ENTER || tecla == KEY_ESC || tecla == KEY_Q ||
            tecla == KEY_0) {
          list_canceled = true;
          break;
        }
        ...
        if (tecla == KEY_UP && sel_idx > 0) {
          ...
          strncpy(filename, available_saves[sel_idx].filename,
                  MAX_LINE_BUFFER - 1);
          strncpy(save_name, available_saves[sel_idx].save_name,
                  MAX_NOME_JOGADOR - 1);
          sobrescrever = true;
          break;
        } else if (tecla == KEY_ESC || tecla == KEY_Q || tecla == KEY_0) {
    ...
    snprintf(filename, sizeof(filename), "%s/%s_%s.txt", SAVE_DIR, save_name,
             timestamp);
  }
  ...
  snprintf(data_to_checksum, sizeof(data_to_checksum),
           ...
           " %c | %c | %c \n"
           "Jogador Atual: %c\n",
           save_name, GAME_VERSION, timestamp_now, (int)game->current_mode,
           game->ai_difficulty, game->nomeJogadorX, game->nomeJogadorO,
           game->placarX, game->placarO, game->empates, game->ultimoVencedor,
           game->historicoJogadasX[0], game->historicoJogadasX[1],
           game->historicoJogadasX[2], game->historicoJogadasO[0],
           game->historicoJogadasO[1], game->historicoJogadasO[2],
           game->numJogadasX, game->numJogadasO, game->tabuleiro[0][0],
           game->tabuleiro[0][1], game->tabuleiro[0][2], game->tabuleiro[1][0],
           game->tabuleiro[1][1], game->tabuleiro[1][2], game->tabuleiro[2][0],
           game->tabuleiro[2][1], game->tabuleiro[2][2], game->jogadorAtual);
...
int listarSaves(SaveInfo *saves_array, int max_saves) {
  ...
  while (fgets(line_filename, sizeof(line_filename), list_file) != NULL &&
         num_saves < max_saves) {
    ...
    if (save_file != NULL) {
      ...
      while (fgets(temp_line, sizeof(temp_line), save_file) != NULL) {
        ...
        if (strstr(temp_line, "Nome do Salvamento:") != NULL) {
          ...
          if (ts_start)
            sscanf(ts_start + strlen("Data e Hora: "), "%s", temp_timestamp);
          ...
          if (sscanf(temp_line, "Empates: %d", &temp_ties) == 1)
            has_stats = true;
...
bool carregarJogo(GameState *game) {
  ...
  do {
    ...
    case KEY_UP:
      selected--;
      if (selected < 0)
        selected = num_available_saves - 1;
      break;

    case KEY_DOWN:
      selected++;
      if (selected >= num_available_saves)
        selected = 0;
      break;
...
save_selected:;
  ...
  while (fgets(line, sizeof(line), file) != NULL) {
    ...
    if (parsing_data_for_checksum) {
      ...
      if (strlen(data_for_checksum) + strlen(line) + 1 > MAX_CHECKSUM_BUFFER) {
        fprintf(stderr,
                "Erro: Buffer de checksum muito pequeno ao carregar jogo.\n");
        fecharArquivo(file);
        return false;
      }
    ...
    if (strstr(line, "Nome do Salvamento:") != NULL) {
      ...
      sscanf(line, "Hist X: %d %d %d", &temp_histX[0], &temp_histX[1],
             &temp_histX[2]);
    } else if (strstr(line, "Hist O:") != NULL) {
      sscanf(line, "Hist O: %d %d %d", &temp_histO[0], &temp_histO[1],
             &temp_histO[2]);
      ...
      if (parsing_data_for_checksum)
        strcat(data_for_checksum, line);
      sscanf(line, " %c | %c | %c ", &temp_tabuleiro[0][0],
             &temp_tabuleiro[0][1], &temp_tabuleiro[0][2]);
      ...
      if (parsing_data_for_checksum)
        strcat(data_for_checksum, line);
      sscanf(line, " %c | %c | %c ", &temp_tabuleiro[1][0],
             &temp_tabuleiro[1][1], &temp_tabuleiro[1][2]);
      ...
      if (parsing_data_for_checksum)
        strcat(data_for_checksum, line);
      sscanf(line, " %c | %c | %c ", &temp_tabuleiro[2][0],
             &temp_tabuleiro[2][1], &temp_tabuleiro[2][2]);
  ...
  if (strlen(temp_playerX) == 0 || strlen(temp_playerO) == 0 ||
      temp_jogadorAtual == ' ' || temp_tabuleiro[0][0] == '?') {
    printf("Erro: Dados essenciais do jogo não foram encontrados ou estão "
  ...
  printf("Placar carregado: %s (X): %d | %s (O): %d | Empates: %d\n",
         game->nomeJogadorX, game->placarX, game->nomeJogadorO, game->placarO,
         game->empates);
...
delete_selected:;
  ...
  printf("\nTem certeza que deseja deletar '%s'? (1 - Sim, 0 - Não): ",
         available_saves[selected].save_name);
