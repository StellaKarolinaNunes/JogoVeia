...
void executarReplay(const GameState *game) {
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
      replay_state.tabuleiro[i][j] = (char)('0' + count++);

  char curr_p =
      'X'; // Assume X começou (ou pode ser dinâmico, mas simplificando)
  // Na verdade, precisamos saber quem começou. Vamos assumir que o histórico
  // registra a ordem exata.
  ...
  for (int i = 0; i < game->total_moves_count; i++) {
    ...
    char replayMsg[100];
    snprintf(replayMsg, sizeof(replayMsg),
             "Reproduzindo jogada %d/%d (Jogador %c na posição %d)", i + 1,
             game->total_moves_count, curr_p, pos);
...
void verificarEAtribuirMedalhas(const char *nome, bool venceu, bool empate,
                                bool contra_ia, int dificuldade) {
  if (strcmp(nome, "IA") == 0 || strcmp(nome, "Nenhum") == 0)
    return;

  int idx = -1;
  for (int i = 0; i < num_ranking_entries; i++) {
    if (strcmp(ranking[i].name, nome) == 0) {
      idx = i;
      break;
    }
  }
...
bool verificarVitoria(const GameState *game, int *winning_line_indices) {
  ...
  for (int i = 0; i < 3; i++) {
    if (game->tabuleiro[i][0] == game->tabuleiro[i][1] &&
        game->tabuleiro[i][1] == game->tabuleiro[i][2]) {
      if (winning_line_indices != NULL) {
  ...
  for (int j = 0; j < 3; j++) {
    if (game->tabuleiro[0][j] == game->tabuleiro[1][j] &&
        game->tabuleiro[1][j] == game->tabuleiro[2][j]) {
      if (winning_line_indices != NULL) {
  ...
  if ((game->tabuleiro[0][0] == game->tabuleiro[1][1] &&
       game->tabuleiro[1][1] == game->tabuleiro[2][2])) {
    if (winning_line_indices != NULL) {
  ...
  if ((game->tabuleiro[0][2] == game->tabuleiro[1][1] &&
       game->tabuleiro[1][1] == game->tabuleiro[2][0])) {
    if (winning_line_indices != NULL) {
...
bool fazerJogada(GameState *game, int jogada) {
  ...
  if (game->tabuleiro[linha][coluna] != 'X' &&
      game->tabuleiro[linha][coluna] != 'O') {
...
bool obterNomesJogadores(GameState *game, GameMode mode) {
  ...
  while (!confirmados) {
    ...
    if (strlen(game->nomeJogadorX) == 0)
      strcpy(game->nomeJogadorX, "Jogador 1");
    ...
    if (mode == MODE_PLAYER_VS_PLAYER || mode == MODE_INFINITE) {
      ...
      if (strlen(game->nomeJogadorO) == 0)
        strcpy(game->nomeJogadorO, "Jogador 2");
    } else {
    ...
    while (!confirm_chosen) {
      exibirTelaConfirmarJogadores(game->nomeJogadorX, game->nomeJogadorO,
                                   selected_confirm);
...
void sortearPrimeiroJogador(GameState *game) {
  ...
  int sorteado =
      (dadoJogador1 > dadoJogador2) ? 0 : 1; // 0 -> JogadorX, 1 -> JogadorO
  char *nomeVencedorSorteio =
      (sorteado == 0) ? game->nomeJogadorX : game->nomeJogadorO;
  ...
  if (sorteado == 1 && strcmp(game->nomeJogadorO, "IA") == 0) {
    exibirTelaSorteio(dadoJogador1, dadoJogador2, game->nomeJogadorX,
                      game->nomeJogadorO, nomeVencedorSorteio, -1);
    esperarEnter();
    escolhaSimbolo = (rand() % 2) + 1; // IA automatically picks 1 or 2
  } else {
    while (!confirm_chosen) {
      exibirTelaSorteio(dadoJogador1, dadoJogador2, game->nomeJogadorX,
                        game->nomeJogadorO, nomeVencedorSorteio, selected);
      KeyCode tecla = lerTeclaMenu();
...
int minimax(GameState *game, int depth, bool isMaximizing, char ai_char,
            char opponent_char, int alpha, int beta) {
  int winning_line[3];
  ...
  if (isMaximizing) {
    ...
    for (int pos = 1; pos <= 9; pos++) {
      ...
      if (game->tabuleiro[r][c] != 'X' && game->tabuleiro[r][c] != 'O') {
        ...
        int score = minimax(game, depth + 1, false, ai_char, opponent_char,
                            alpha, beta);
    ...
    for (int pos = 1; pos <= 9; pos++) {
      ...
      if (game->tabuleiro[r][c] != 'X' && game->tabuleiro[r][c] != 'O') {
        ...
        int score =
            minimax(game, depth + 1, true, ai_char, opponent_char, alpha, beta);
        game->tabuleiro[r][c] = original;
        if (score < bestScore)
          bestScore = score;
        if (score < beta)
...
int obterJogadaIA(GameState *game) {
  ...
  if (game->ai_difficulty == DIFFICULTY_HARD) {
    ...
    for (int pos = 1; pos <= 9; pos++) {
      ...
      if (game->tabuleiro[r][c] != 'X' && game->tabuleiro[r][c] != 'O') {
        ...
        game->tabuleiro[r][c] = ai_char;
        int score =
            minimax(game, 0, false, ai_char, opponent_char, -1000, 1000);
        game->tabuleiro[r][c] = original;
        if (score > bestScore) {
          bestScore = score;
          move = pos;
  ...
  for (int pos = 1; pos <= 9; pos++) {
    ...
    if (game->tabuleiro[linha][coluna] != 'X' &&
      ...
      int dummy[3];
      if (verificarVitoria(&temp, dummy))
        return pos;
    }
  }
  for (int pos = 1; pos <= 9; pos++) {
...
void funcaoJogar(GameState *game, bool is_resumed_game) {
  ...
  if (!is_resumed_game) {
    ...
    // selected_mode corresponds to indices 0: PVP, 1: PVE, 2: INFINITE, 3:
    // VOLTAR
    if (selected_mode == 3) {
      ...
      game->current_mode = MODE_INFINITE;
    ...
    while (!tourney_chosen) {
      ...
      switch (tecla) {
      case KEY_UP:
        if (selected_tourney == 0)
          selected_tourney = 3;
        else
          selected_tourney--;
        break;
      case KEY_DOWN:
        if (selected_tourney == 3)
          selected_tourney = 0;
        else
          selected_tourney++;
        break;
    ...
    if (selected_tourney == 3)
      return; // VOLTAR AO MENU PRINCIPAL
    else if (selected_tourney == 1)
      game->tournament_target_wins = 3;
    else if (selected_tourney == 2)
      game->tournament_target_wins = 5;
    else
      game->tournament_target_wins = 0;

    if (!obterNomesJogadores(game, game->current_mode))
      return;
  }
  if (is_resumed_game) {
    // Correção: Se o modo carregado for PVP mas o Jogador O for a IA, mudar
    // para modo IA
    if (game->current_mode == MODE_PLAYER_VS_PLAYER &&
        strcmp(game->nomeJogadorO, "IA") == 0) {
      game->current_mode = MODE_PLAYER_VS_AI;
    }
    if (!obterNomesJogadores(game, game->current_mode))
      return;
  ...
  while (jogarNovamente) {
    ...
    if (is_resumed_game) {
      if (!(verificarVitoria(game, winning_line) ||
            (game->current_mode != MODE_INFINITE && jogadasFeitas == 9))) {
        is_resumed_game = false;
      }
      // Note: Se for terminal, is_resumed_game continua true para pular o
      // sorteio/inicialização no início da rodada, mas a rodada terminada
      // resetará tudo.
    }
    ...
    while (!rodadaTerminada) {
      ...
      if (verificarVitoria(game, winning_line)) {
        ...
        printf("╠══════════════════════════════════════════════════════════════"
               "════════════════════════════════════════╣\n");
        printf("║                                  ⚖️  Este jogo terminou em "
               "EMPATE! ⚖️                                 ║\n");
        printf("╚══════════════════════════════════════════════════════════════"
               "════════════════════════════════════════╝\n\n");
        rodadaTerminada = true;
      ...
      char *nome_jogador_atual =
          (game->jogadorAtual == 'X') ? game->nomeJogadorX : game->nomeJogadorO;
      int escolhaAcao = -1;

      if (game->current_mode == MODE_PLAYER_VS_AI &&
          strcmp(nome_jogador_atual, "IA") == 0) {
        exibirInterfaceJogo(game, NULL, "A IA está pensando...", -1);
        escolhaAcao = obterJogadaIA(game);
        char iaMsg[100];
        snprintf(iaMsg, sizeof(iaMsg), "A IA (%s) jogou na posição %d.",
                 nome_jogador_atual, escolhaAcao);
        ...
        while (true) {
          char turnMsg[100];
          snprintf(turnMsg, sizeof(turnMsg), "É a vez do jogador %s (%c)",
          ...
          if (tecla == KEY_UP) {
            ...
            if (selected_cell % 3 != 0)
              selected_cell -= 1;
          } else if (tecla == KEY_RIGHT) {
            if (selected_cell % 3 != 2)
              selected_cell += 1;
      ...
      if (escolhaAcao == 0) {
        ...
        if (fazerJogada(game, escolhaAcao)) {
          ...
          if (verificarVitoria(game, winning_line)) {
            ...
            verificarEAtribuirMedalhas(nome_jogador_atual, true, false,
                                       contra_ia, game->ai_difficulty);

            char *winner_name = nome_jogador_atual;
            char *loser_name = (game->jogadorAtual == 'X') ? game->nomeJogadorO
                                                           : game->nomeJogadorX;
            ...
            // REPLAY PROMPT
            if (perguntarReplayResultado(nome_jogador_atual, game->jogadorAtual,
                                         false)) {
              executarReplay(game);
            }

          } else if (game->current_mode != MODE_INFINITE &&
                     jogadasFeitas == 9) {
            game->empates++;
          ...
          exibirInterfaceJogo(game, NULL, "Posição inválida ou já ocupada!",
                              -1);
          delaySimples(1500);
        }
      }
